...
RayCanary is already officially supported on [several devices](./supported-devices.md), and people are often interested in adding support for hardware they already own. Here's a non-exhaustive list of situations where we'd consider adding a new Tier 2 device:
...
That said, you can always maintain a fork, or install RayCanary manually without writing an installer. You can promote this work in the [GitHub discussions](https://github.com/GreerBK/raycanary/discussions) area, where most new hardware investigations happen.
...
RayCanary is a Linux binary that reads traffic from the Qualcomm diagnostic interface, which requires root. If either of those isn't available, RayCanary can't work. Everything else (displays, buttons) is secondary, and we can deal with it later.

In the devices we currently support `/dev/diag` is the interface for Qualcomm diagnostics and devices with this will be easiest to support. Newer Qualcomm modems expose the diagnostic interface over a USB gadget which is something we are working on support for, but do not currently have. Thus devices with the former diagnostic interface will be easier to port RayCanary to.
...
Getting a root shell varies from device to device. Check the [GitHub discussions](https://github.com/GreerBK/raycanary/discussions) for prior art, and look through the installer source in `installer/src/` for inspiration. These approaches are common:
...
## Installing RayCanary manually

The RayCanary installation consists of just two components: the `raycanary-daemon` binary, and the config file (`config.toml`).
...
Then, `./raycanary-daemon config.toml` can be started manually.

You can refer to [Installing from source](./installing-from-source.md) for how to obtain the `raycanary-daemon` binary.
...
You can copy the daemon and config files to the device using `netcat` or `adb push`. They don't have to be in `/data/raycanary/`, this is just convention. If you use a different path, be sure to update the `qmdl_store_path` setting in `config.toml`.
...
To make RayCanary start on boot, you'll need an init script. The existing installers use the template at `dist/scripts/raycanary_daemon`, which has a `#RAYCANARY-PRESTART` placeholder that gets replaced with device-specific setup commands (e.g. killing a vendor UI process, mounting an SD card). Look at how the existing installers handle this in their `install()` functions.

## Display support

The `device` setting [mentioned above](#installing-raycanary-manually) also controls which display driver is loaded (see [`Device` enum in `lib/src/lib.rs`](https://github.com/GreerBK/raycanary/blob/main/lib/src/lib.rs)). Unless your device is a variant of an existing device, you'll want to add a new variant to the `Device` enum and write a corresponding display module in `daemon/src/display/`.
...
RayCanary can use the power button to restart recordings via a double-tap gesture. The implementation is in [`daemon/src/key_input.rs`](https://github.com/GreerBK/raycanary/blob/main/daemon/src/key_input.rs). It currently has no structure for device-specific implementations, as all devices we support expose the same input event interface.
...
At this point you'll want to have figured out how to automate the entire installation in principle, and how to make it as repeatable as possible. A proof-of-concept of this in bash or another language is also a welcome contribution (to be posted on [GitHub discussions](https://github.com/GreerBK/raycanary/discussions), not as a PR).

Writing the installer means adding a new variant to the `Command` enum in [`installer/src/lib.rs`](https://github.com/GreerBK/raycanary/blob/main/installer/src/lib.rs) and implementing the install logic in a new module under `installer/src/`. Each subcommand maps to a device-specific entry point function (e.g. `tplink::main_tplink`, `orbic_network::install`).

The installer gets the daemon binary path from `env!("FILE_RAYCANARY_DAEMON")`, which is set at build time. Config installation is handled by the shared `install_config()` helper in the `connection` module, which writes the config file with the correct device name.
...
Please reuse existing utilities wherever possible. Take a look at [`installer/src/tplink.rs`](https://github.com/GreerBK/raycanary/blob/main/installer/src/tplink.rs) and [`installer/src/orbic_network.rs`](https://github.com/GreerBK/raycanary/blob/main/installer/src/orbic_network.rs) for inspiration. But the structures there are still evolving, and we'll happily guide you during code review.
